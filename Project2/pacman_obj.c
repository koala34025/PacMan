...
static bool pacman_movable(Pacman* pacman, Map* M, Directions targetDirec) {
	...
	*/
	...
	{
	case UP:
		if (is_wall_block(M, pacman->objData.Coord.x, pacman->objData.Coord.y-1) ||
			is_room_block(M, pacman->objData.Coord.x, pacman->objData.Coord.y-1))
			return false;
		break;
	case DOWN:
		if (is_wall_block(M, pacman->objData.Coord.x, pacman->objData.Coord.y+1) ||
		...
		break;
	case LEFT:
		if (is_wall_block(M, pacman->objData.Coord.x-1, pacman->objData.Coord.y) ||
			is_room_block(M, pacman->objData.Coord.x-1, pacman->objData.Coord.y))
			return false;
		break;
	case RIGHT:
		if (is_wall_block(M, pacman->objData.Coord.x+1, pacman->objData.Coord.y) ||
	...
	/*
	if (is_wall_block(M, pacman->objData.Coord.x, pacman->objData.Coord.y) ||
		is_room_block(M, pacman->objData.Coord.x, pacman->objData.Coord.y))
		return false;
	*/
	return true;
}
