...
bool ghost_movable(Ghost* ghost, Map* M, Directions targetDirec, bool room) {
	...
	{
	case UP:
		if (is_wall_block(M, ghost->objData.Coord.x, ghost->objData.Coord.y-1)
		|| (room && is_room_block(M, ghost->objData.Coord.x, ghost->objData.Coord.y-1)))
		return false;
		break;
	case DOWN:
		if (is_wall_block(M, ghost->objData.Coord.x, ghost->objData.Coord.y+1)
		...
		break;
	case LEFT:
		if (is_wall_block(M, ghost->objData.Coord.x-1, ghost->objData.Coord.y)
		|| (room && is_room_block(M, ghost->objData.Coord.x-1, ghost->objData.Coord.y)))
		return false;
		break;
	case RIGHT:
		if (is_wall_block(M, ghost->objData.Coord.x+1, ghost->objData.Coord.y)
